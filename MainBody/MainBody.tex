% 文件名：Mainbody.tex
% 文件描述：以 scuthesis 四川大学学位论文文档类为基础的 LaTeX 模版
% 作者：LegendaryLeo, GuoQuan
% 修改日期：2017年3月1日

% 选用scuthesis文档类，参数master为硕士论文，可选doctor和bachelor，分别对应博士和学士
\documentclass[master,UTF8]{scuthesis}

% 设置文档信息
\title{论文标题}
% 文档封面专用标题，支持封面标题换行，使用 '\\' 命令换行
\CoverTitle{论文标题}
\ENGtitle{The Dissertation Title}
\author{作者}
\ENGauthor{Author Name}
\accomplishdate{论文完成日期}
\school{学院名称}
\supervisor{指导教师{\quad}教授/副教授}
\ENGsupervisor{Prof. Supervisor}
\major{专业名称}
\ENGmajor{Major Name}
\direction{方向名称}
\ENGdirection{Direction Name}
\defensedate{答辩日期}
\keywords{关键词1，关键词2，关键词3，关键词4，关键词5}
\ENGkeywords{Keyword1, Keyword2, Keyword3, Keyword4, Keyword5}

\begin{document}
% 论文正文前
\frontmatter
% 自动制作标题
\maketitle
% 包含摘要
\include{Chapters/0_0_Abstract}
% 包含缩略词表
\include{Chapters/0_1_Abbreviations}
% 包含符号表
\include{Chapters/0_2_Symbols}
% 自动制作目录
\maketoc

% 论文正文
\mainmatter
% 包含第一章、第二章等等
\include{Chapters/Introduction}
\include{Chapters/Chapter2}

% 论文正文后
\backmatter
% 按国标自动制作参考文献
\begin{reference}
	% 参考文献数据文件为本目录下的ReferenceBase.bib
	\bibliographystyle{GBT7714-2005NLang-UTF8}\bibliography{ReferenceBase}
\end{reference}
% 包含在读期间科研成果
\include{Chapters/Achievements}
% 生成论文原创性声明
\originalstate
% 生成论文版权授权书
\copyrightauth
% 包含致谢
\include{Chapters/Acknowledgement}
\end{document}
